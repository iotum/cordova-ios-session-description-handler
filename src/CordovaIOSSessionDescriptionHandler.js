 ...
 * interface with WebRTC in a way that is compatible with the implemenatation from
 * cordova's iOS rtc plugin.
 */
class CordovaIOSSessionDescriptionHandler extends SIP.SessionDescriptionHandler {
  constructor(logger, session, options) {
    super();
    ...
    this.logger.log(
      'SessionDescriptionHandlerOptions: ' + JSON.stringify(this.options)
    );
    ...
    this.WebRTC = {
      ...
      getUserMedia: environment.navigator.mediaDevices.getUserMedia.bind(
    ...
    this.constraints = this.checkAndDefaultConstraints(
      this.options.constraints
    );
  }

  static factory(session, options) {
    const logger = session.ua.getLogger(
      'sip.invitecontext.sessionDescriptionHandler',
      session.id
    );
    return new CordovaIOSSessionDescriptionHandler(logger, session, options);
  }
  ...
  setDescription(sessionDescription, options, modifiers) {
    ...
    description.type = this.hasOffer('local') ? 'answer' : 'offer';
    ...
    return Promise.resolve()
      ...
      .catch(e => {
        ...
        if (
          /^m=video.+$/gm.test(sessionDescription) &&
          !options.disableAudioFallback
        ) {
          ...
          return this.setDescription(
            sessionDescription,
            options,
            [SIP.Web.Modifiers.stripVideo].concat(modifiers)
          );
        }
        const error = new SIP.Exceptions.SessionDescriptionHandlerError(
          'setDescription',
  ...
  sendDtmf(tones, options) {
    ...
    try {
      ...
      if (
        e.type === 'InvalidStateError' ||
        e.type === 'InvalidCharacterError'
      ) {
        this.logger.error(e);
        return false;
      } else {
  ...
  createOfferOrAnswer(RTCOfferOptions, modifiers) {
    ...
    return pc[methodName](RTCOfferOptions)
      ...
      .then(sdp => SIP.Utils.reducePromises(modifiers, sdp))
      .then(sdp => {
        this.resetIceGatheringComplete();
      ...
      .then(() => {
        return SIP.Utils.reducePromises(
          modifiers,
  ...
  initPeerConnection(options) {
    ...
    options.rtcConfiguration = this.addDefaultIceServers(
    ...
    this.peerConnection = new this.WebRTC.RTCPeerConnection(
      options.rtcConfiguration
    );
    ...
    this.peerConnection.onicegatheringstatechange = function() {
      self.logger.log(
        'RTCIceGatheringState changed: ' + this.iceGatheringState
      );
